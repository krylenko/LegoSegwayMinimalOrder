...
// ECE542 project code - daniel ford spring 2013

#include "setup.h"

// PID controller
void ctrlPID(float error){

  errSum += error;
  intTerm = Ki*errSum;
  if(intTerm > sumMax) intTerm = sumMax;
  if(intTerm < sumMin) intTerm = sumMin;

  float rawOut = Kp*error + intTerm + Kd*(error-errLast);
  leftV = (int)rawOut*MOTORRNG;
  rightV = leftV;
  errLast = error;
